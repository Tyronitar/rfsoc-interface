...
if __name__ == '__main__':
    ...
    # Mid-field set 1008 H-pol
    mid_field_date = '20260805'
    new_mf_vpol_file = h5py.File('Mid_field_tile_2_set1008_beammap.h5', mode='w')
    mf_vpol = ProcessedData.load(mid_field_date, 1008)
    new_mf_vpol_file.attrs['dpix']= mf_vpol['map'].attrs['dpix']
    new_mf_vpol_file.attrs['units']= mf_vpol['map'].attrs['units']
    new_mf_vpol_file.create_dataset('chanmask', data=mf_vpol.chanmask)
    mf_vpol.file.copy('map/map_val', new_mf_vpol_file)
    mf_vpol.file.copy('map/map_az', new_mf_vpol_file)
    mf_vpol.file.copy('map/map_za', new_mf_vpol_file)
    mf_vpol.file.copy('beammap', new_mf_vpol_file)
    new_mf_vpol_file.close()