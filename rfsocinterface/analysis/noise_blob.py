...
def line_and_circle_intersection_points(m, b, x0, y0, r):
    """Find the intersection of a line an circle."""
    x_list = []
    y_list = []
...
def plot_angle_in_blob(
    ...
    title: str = '',
    fit_order: int = 2,  # noqa: ARG001
    alpha: float = 0.5,
    sigma: float = 2.5,
    markersize: float = 0.5,
    source_crossing_sample: int | None = None,
) -> Figure:
    ...
    iq_inliers_mask = (data_IQ >= iq_mean - sigma * iq_std) & (
        data_IQ <= iq_mean + sigma * iq_std
    )
    ...
    fd_inliers_mask = (data_freq_diss >= fd_mean - sigma * fd_std) & (
        data_freq_diss <= fd_mean + sigma * fd_std
    )
    ...
    min_x = min(min_iq[0], min_fd[0])  # noqa: F841
    max_x = max(max_iq[0], max_fd[0])
    min_y = min(min_iq[1], min_fd[1])  # noqa: F841
    max_y = max(max_iq[1], max_fd[1])
    med_plot_dim = abs(min(max_x, max_y)) / 2
    med_plot_dim_iq = abs(np.min(max_iq)) / 2  # noqa: F841
    med_plot_dim_fd = abs(np.min(max_fd)) / 2  # noqa: F841
    ...
    if source_crossing_sample is None:
        ...
        iq_angle_label = (
            f'IQ angle based on PCA ($\\theta = {np.degrees(iq_angle):.2f}^\\circ$)'
        )
        ...
        fd_angle_label = (
            f'Freq / Diss angle based on PCA '
            f'($\\theta = {np.degrees(fd_angle):.2f}^\\circ$)'
        )
    else:
        ...
        iq_angle_label = (
            f'IQ angle implied by source crossing \n'
            f'($\\theta = {np.rad2deg(iq_angle):.2f}^\\circ$)'
        )
        ...
        fd_angle_label = (
            f'Freq / Diss angle implied by source crossing '
            f'\n($\\theta = {np.rad2deg(fd_angle):.2f}^\\circ$)'
        )

    # Plot the vectors
    iq_vec_plot = ax.quiver(
        0,
        0,
        scaled_iq_vec[0],
        scaled_iq_vec[1],
    ...
    plot_arc(
        ...
        color='black',
        label=(
            f'Actual rotation used to create Freq / Diss data \n'
            f'($\\theta = {np.rad2deg(IQ_to_freq_diss_angle):.2f}^\\circ$)'
        ),
    )
