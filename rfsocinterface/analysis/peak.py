...
def check_focus(pdata: ProcessedData, resonators: list[int], primary_direction: str='az', fractional_difference_threshold: float=0.5):
    ...
    max_sample = np.argmax(pdata.data_mK[282])
    ...
    with PdfPages(f'peaks_{pdata.file_stub}.pdf') as pdf:
        ...
        for i_res in resonators:
            ...
            if pdata.date == '20260427' and pdata.setnum == 1010 and i_res in [226, 282, 358, 378, 956]:
                old_data = ProcessedData.load('20260427', 1009, mode='r')
                # Use the olf data for finding where the peak should be
                data_segment = old_data.data_mK[i_res, samples]
                ...
                # Use the actual data for fitting
                data_segment = pdata.data_mK[i_res, samples]
            else:
                data_segment = pdata.data_mK[i_res, samples]
                right_peak_idx = right_indices[np.argmax(data_segment[right_indices])]
                left_peak_idx = left_indices[np.argmax(data_segment[left_indices])]
            ...
            amplitude_guess = np.max(pdata.data_mK[i_res])
            ...
            right_patch = mpatches.Patch(color='orange', label=f'Amplitude = {amplitude_right:.3e} mK, FWHM = {fwhm_right:.3f} deg')
            ...
            left_patch = mpatches.Patch(color='green', label=f'Amplitude = {amplitude_left:.3e} mK, FWHM = {fwhm_left:.3f} deg')
