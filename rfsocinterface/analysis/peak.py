...
def check_focus(
    ...
    with PdfPages(f'peaks_{pdata.file_stub}.pdf') as pdf:
        for i_res in resonators:
            ...
            samples_1 = np.where((relative_pos >= 0.5) & ~np.isnan(telescope_pos))[0][
                -1
            ]
            ...
            plt.title(
                f'Detector {i_res} Peak Finding (Polarization {pdata.detector_pol[i_res]})'
            )
            plt.plot(telescope_pos[:], data_segment, label='Full Trace', color='b')
            plt.plot(
                telescope_pos[right_fit_ind],
                data_segment[right_fit_ind],
                label=f'Right (${{{primary_direction.upper()}}}_0$ = {right_az_0:.3f})',
                color='orange',
            )
            ...
            right_patch = mpatches.Patch(
                color='orange',
                label=f'Amplitude = {amplitude_right:.3e} {units}, FWHM = {fwhm_right:.3f} deg',
            )

            plt.plot(
                telescope_pos[left_fit_ind],
                data_segment[left_fit_ind],
                label=f'Left (${{{primary_direction.upper()}}}_0$ = {left_az_0:.3f})',
                color='green',
            )
            ...
            left_patch = mpatches.Patch(
                color='green',
                label=f'Amplitude = {amplitude_left:.3e} {units}, FWHM = {fwhm_left:.3f} deg',
            )
            ...
            plt.xlabel(
                f'{"Azimuth" if primary_direction.lower() == "az" else "Zenith Angle"} (degrees)'
            )
...
class FindFWHM(DataRoutine):
    ...
    def run(self, pdata: ProcessedData, inputs: list[str] | None = None):
        ...
        for i_res in resonators:
            ...
            plt.plot(
                telescope_pos[fit_region],
                gaussian_profile(res.x, telescope_pos[fit_region]),
