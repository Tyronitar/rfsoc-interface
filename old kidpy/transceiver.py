...
class Transceiver:
    def __init__(self, comport) -> None:
        ...
        if self.ser.is_open:
            ...
            if resp.strip() == b"transceiver_3.2.0":
                print("Connected")
            else:
                self.ser.close()
    ...
    def set_atten(self, addr:int, value : float):
        ...
        if _ASSERTIONS:
            ...
            assert addr > 0 and addr <= 4, "Address out of range (1 through 4)"
...
if __name__ == "__main__":
    ...
    print("\nTESTING FOR GRACEFUL FAIL DUE TO INVALID ADDRESS 0")
    result = ""
    status, msg = t.set_atten(0, 0.0)
    if not status and msg == "FAIL, BAD ADDRESS NOT BETWEEN 1 THROUGH 4":
        result = f"tring to set attenuation for {0};\t\t"
        result = result + _PASS
    else:
        result = result + _FAIL
    print(result)
